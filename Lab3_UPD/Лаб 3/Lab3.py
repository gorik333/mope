...
class Lab3:
    ...
    def student_criteria(self, m, N, y_table, factors_table):
        ...
        self.coefficients_beta_s = np.array([np.average(self.average_Y*x_i[i]) for i in range(len(x_i))])

        print("Оцінки коефіцієнтів βs: " + ", ".join(list(map(str,self.coefficients_beta_s))))
        t_i = np.array([abs(self.coefficients_beta_s[i])/standard_deviation_beta_s for i in range(len(self.coefficients_beta_s))])
        ...
        betas_to_print = list(itertools.compress(self.coefficients_beta_s, self.importance))

        print("{0[0]} {0[1]} {1[0]} {1[1]} {2[0]} {2[1]} {3[0]} {3[1]}".format(*to_print))
        self.equation = " ".join(["".join(i) for i in zip(list(map(lambda x: "{:+.2f}".format(x), betas_to_print)),x_i_names)])
    ...
    def fisher_criteria(self, m, N, d, factors_table, matrix, b_coefficients, importance):
        ...
        for i in range(4):
            if self.importance[i]==True:
                print(self.coefficients_beta_s[i],"- значимий член")
        print("Рівняння регресії без незначимих членів: y = " + self.equation)
